...
/**
 * Checkout master if necessary, then pull latest.
 */
async function checkoutAndPullMaster() {
 ...
 */
async function verifyNoConflictingTag(packageVersion) {
 ...
 */
async function checkChangelog(packageVersion) {
  const answer = await inquirer.prompt(constructFilenamePrompt());

  // Remember the filename for next time.
  prefs.projects[currentPath].changelogFilename = answer.logFilename;

  try {
    const changelogContents = fs.readFileSync(answer.logFilename, { encoding: 'utf8' });
  }
...
export async function run(returnToMenu, exitWithError) {
  ...
  await verifyNoConflictingTag(packageFile);
  
  await checkChangelog(packageVersion);
